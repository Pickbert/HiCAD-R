...
export async function* createGenerateEvents(options: GeneratePipelineOptions): AsyncGenerator<AiStreamEvent> {
  yield { type: 'start', provider: options.provider, model: options.model, message: `请求 ${options.provider}:${options.model} 解析设计意图` };
  try {
    if (!options.adapter) {
    ...
    yield { type: 'code', provider: options.provider, model: options.model, code, source: 'provider', message: 'provider-codegen' };
...
export async function* createFallbackGenerateEvents(options: {
  ...
  yield { type: 'fallback', provider: options.provider, model: options.model, source: 'fallback', message: '真实模型不可用，使用本地确定性 codegen fallback' };
  ...
  yield { type: 'code', provider: options.provider, model: options.model, code, source: 'fallback', message: 'fallback-local-codegen' };
  yield { type: 'done', provider: options.provider, model: options.model, source: 'fallback', message: 'fallback 生成完成' };
}
