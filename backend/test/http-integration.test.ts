...
describe('HiCAD HTTP integration', () => {
  ...
  it('covers registration, login, refresh token, and /users/me', async () => {
    ...
    const login = await http.request
      .post('/api/auth/login')
      .send({ email: registered.user.email, password: 'password123' })
      .expect(201);
  ...
  it('covers model CRUD, publish, unpublish, share, and public share preview', async () => {
    ...
    const updated = await http.request
      ...
      .set('Authorization', bearer(auth.accessToken))
      .send({
        title: 'Updated HTTP Box',
        description: 'updated',
        code: 'const boxWidth = 120 // 宽度 unit:mm min:10 max:200',
        category: 'workspace',
        tags: ['http'],
        material: 'steel'
      })
    ...
    const published = await http.request
      .post(`/api/models/${created.body.id}/publish`)
      .set('Authorization', bearer(auth.accessToken))
    ...
    const share = await http.request
      .post(`/api/models/${created.body.id}/share`)
      .set('Authorization', bearer(auth.accessToken))
      .expect(201);
    expect(share.body.token).toBeTruthy();
    await http.request
      .get(`/api/models/share/${share.body.token}`)
      .expect(200)
      .expect((res) => {
        expect(res.body.id).toBe(created.body.id);
      });

    const unpublished = await http.request
      .post(`/api/models/${created.body.id}/unpublish`)
      .set('Authorization', bearer(auth.accessToken))
      .expect(201);
    expect(unpublished.body.visibility).toBe('private');

    await http.request
      .delete(`/api/models/${created.body.id}`)
      .set('Authorization', bearer(auth.accessToken))
      .expect(200);
  ...
  it('covers AI SSE fallback event sequence', async () => {
    ...
    const response = await http.request
      .get('/api/ai/generate?prompt=make%20a%20box&provider=deepseek')
      .buffer(true)
      .parse(collectSse)
      .expect(200);
  ...
  it('covers admin permission boundaries and activation code management', async () => {
    ...
    const created = await http.request
      ...
      .set('Authorization', bearer(admin.accessToken))
      .send({ code: 'HTTP-CODE', tier: 'team', maxUses: 2 })
      .expect(201);
  ...
  it('rejects forged payment callbacks through the HTTP API', async () => {
    ...
    await http.request
      ...
      .set('x-hicad-signature', 'forged')
      .send({ orderNo: order.body.orderNo, amount: 100, status: 'paid' })
      .expect(401);
  });
});
