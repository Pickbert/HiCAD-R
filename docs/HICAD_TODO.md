...
- [x] 前端接入真实登录态持久化和刷新 token。
- [x] 登录/注册从顶栏临时输入改成正式弹窗或独立页面。
- [x] 工作台保存/发布/分享失败时展示明确错误。
- [x] 模型列表：我的模型、草稿、已发布、分享中。
- [x] 市场页独立路由或独立视图，支持搜索、分类、标签、排序。
- [x] 分享页独立路由，未登录可只读预览。
- [x] 管理后台页面：用户、模型、模板、订单、反馈、激活码。
- [x] 参数面板支持分组、单位显示、数值校验和重置默认值。
- [x] Monaco 增加 JSCAD snippets、自动补全、错误标记。
- [x] Monaco 对 AI 生成代码支持 diff/apply，而不是直接覆盖。
- [x] AI 修改模式支持“基于当前代码修改”并显示变更摘要。
- [x] AI 消息合并流式 delta，避免每个 chunk 独立成消息气泡。
- [x] AI 面板增加历史记录和重新生成。
- [x] 模型生成时展示 `start/spec/retry/code/done/error` 细粒度状态。
- [x] 视图模式完善：实体、线框、X-Ray、平面 CAD。
- [x] 标注开关：尺寸标注、参数标签、坐标轴、网格。
- [x] 材质面板完善 10+ 材质，并支持模型级/部件级材质。
- [x] 支持 STL 导入，Base64 存储并进入同一保存/发布流程。
- [x] 支持 OBJ 导出。
- [x] 支持导出前预览导出体积、三角面数量、单位。
- [x] 移动端改为可折叠面板，避免三栏布局挤压。
- [x] 前端增加 toast、confirm、loading、empty、error 通用组件。
- [x] 前端增加可访问性：按钮 aria-label、键盘操作、焦点态。

## P1 CAD 与渲染

- [x] 真实执行 JSCAD `main()` 并捕获异常。
- [x] 支持常用建模 API：`cuboid`、`roundedCuboid`、`cylinder`、`sphere`、`union`、`subtract`、`translate`、`rotate`、`colorize`。
- [x] 支持复杂模型的部件树和颜色分组。
- [x] Worker 使用 Transferable Objects 返回几何数据，减少复制。
- [x] 主线程合并同材质 Mesh，降低 Draw Call。
- [x] 计算并展示包围盒尺寸。
- [x] 计算并展示面数、顶点数、渲染耗时。
- [x] 大模型降级策略：限制面数、提示优化、暂停自动渲染。
- [x] WebGL context lost 恢复处理。
- [x] 相机控制：旋转、平移、缩放、重置视角。
- [x] 预设视角：正视、侧视、顶视、等轴测。
- [x] 支持阴影、环境光和工程网格。
- [x] 平面 CAD 模式显示正交投影和尺寸线。
- [x] X-Ray 模式透明材质和边线叠加。
- [x] STL/OBJ 导出使用真实网格数据。

## P1 测试与质量

- [x] 后端集成测试覆盖注册、登录、刷新 token。
- [x] 后端集成测试覆盖模型 CRUD、发布、撤回、分享。
- [x] 后端集成测试覆盖模板读取和使用。
- [x] 后端集成测试覆盖 AI SSE 事件序列。
- [x] 后端集成测试覆盖管理员权限。
- [x] 前端组件测试覆盖 AI 面板、参数面板、市场面板。
- [x] 前端组件测试覆盖保存、发布、分享按钮行为。
- [x] Worker 测试覆盖危险代码拦截、超时、错误返回。
- [x] CAD 测试覆盖参数解析、参数应用、材质注释、多色分组。
- [x] E2E：生成盒子 -> 预览 -> 调参 -> 保存 -> 发布 -> 分享 -> 导出 STL。
- [x] E2E：普通用户无法访问管理后台。
- [x] E2E：伪造支付回调失败。
- [x] Playwright 截图检查桌面端布局。
- [x] Playwright 截图检查移动端布局。
- [x] 依赖安全审计纳入 CI。
- [x] 构建产物体积检查，重点处理 Monaco 过大 bundle。
